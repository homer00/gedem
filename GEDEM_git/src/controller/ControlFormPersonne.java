...
public class ControlFormPersonne {
	...
	public ControlFormPersonne(ViewFormPersonne vfp, App appli) {
		...
		al_personne = new ArrayList<Personne>();
	...
	public void ajouterPersonne() {
		...
		String val2 = (String)vfp.getJt_nom().getText(); // nom
		String val3 = (String)vfp.getJt_prenom().getText(); // prenom
		String val4 =(String)vfp.getJt_mail().getText(); // mail
		String val5 =(String)vfp.getJt_tel().getText(); // Tel
		String val6 =(String)vfp.getJt_login().getText(); // login
		...
		if (flag_validation ) { // si le formulaire a été correctement renseigné
			...
			try {
				...
				for (int i=0; i<al_personne.size(); i++) { // Affichage dans la console des enregistrements
						System.out.println(al_personne.get(i).toString());
